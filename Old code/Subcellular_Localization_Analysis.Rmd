...
impNEM = fread('DGEresults/Limma_results_table_NEM3Y.csv')
...
ggplot(filter(plot_dat 
              # FDR <= 0.05
              ), aes(reorder(Subcell_loc, fold_enrichment), fold_enrichment, fill = fold_enrichment))+
  geom_col(color = 'black')+
  theme(axis.text.x = element_text(angle = 30, size = 10))+
  # geom_label(aes(label = INF))+
  xlab(NULL)+
...
NEM = ggplot(plot_dat, aes(x = Term_logFC, y = log2(FDR)*-1, color = fold_enrichment))+
  geom_point(aes(size = num_genes, color = fold_enrichment))+
  geom_point(aes(size = num_genes), shape = 1, stroke = 0.25, color = 'black')+
  ggrepel::geom_label_repel(data = filter(plot_dat 
                                          # FDR <= 0.05
                                          ), aes(label = Subcell_loc),
                            size = 4,
                            color = 'black', 
                            box.padding = 0.4,
                            label.padding = 0.1,
