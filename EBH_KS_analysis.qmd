...
print_res$CG_No_filter = print(paste('Total of',
            ...
            'genes mapped to', 
            ks_res_CG %>% pull(Parent_haplotype) %>% unique() %>% length(),
            'Subcellular loci '))
            
print_res$CG_Filter = print(paste('and with fdr <= 0.05', 
            ks_res_CG %>% filter(fdr < 0.05) %>% pull(logFC) %>% unique() %>% length(),
            'genes mapped to', 
            ks_res_CG %>% filter(fdr < 0.05) %>% pull(Parent_haplotype) %>% unique() %>% length(),
            'Subcellular loci '))
```
...
one = ks_res_less_CG %>% 
  ...
  mutate(Parent_haplotype = gsub('with\\sbulged\\sadenosine\\sas\\snucleophile|to\\s', '', Parent_haplotype)) 

# Merge DGE results with GO results
plot_dat = one %>% 
  select(Parent_haplotype, 
         fdr,
         `Number_of_genes`,
         Avg_logFC) %>% 
  distinct() %>%
...
impCG_Enrichment_out = plot_dat %>% 
  mutate(Trait = 'CG')

labs_normal = plot_dat %>% 
  # Make labels for terms containing these strings and fdr less than or equal to 0.05
  filter(Parent_haplotype %like% 'RNA|cell wall|polysaccharide|cellulose|glucan|defense|hormone|jasmonic acid|abscisic acid|salicylic acid|ethylene' &
           fdr <= 0.05) %>% 
  select(Parent_haplotype,
         fdr,
         Avg_logFC,
...
CG = ggplot(top, aes(x = Gene_type, y = reorder(Parent_haplotype, Avg_logFC), color = Avg_logFC))+
...
print_res$PHY_No_filter = print(paste('Total of',
            ...
            'genes mapped to', 
            ks_res_PHY %>% pull(Parent_haplotype) %>% unique() %>% length(),
            'Subcellular loci '))
            
print_res$PHY_Filter = print(paste('and with fdr <= 0.05', 
            ks_res_PHY %>% filter(fdr < 0.05) %>% pull(logFC) %>% unique() %>% length(),
            'genes mapped to', 
            ks_res_PHY %>% filter(fdr < 0.05) %>% pull(Parent_haplotype) %>% unique() %>% length(),
            'Subcellular loci '))
```
...
# Make new table for combining DEG results with GO results and writing out
impPHY_Enrichment_out = plot_dat %>% 
  mutate(Trait = 'PHY')
...
PHY = ggplot(top, aes(x = Gene_type, y = reorder(Parent_haplotype, Avg_logFC), color = Avg_logFC))+
  geom_point(aes(size = log10(fdr)*-1, color = Avg_logFC))+
  geom_point(aes(size = log10(fdr)*-1), shape = 1, stroke = 0.25, color = 'black')+
...
```{r}
# Make vector of unique logFC values
ref_NEM_3Y = impNEM_3Y %>% 
  distinct(GeneID, logFC) %>% 
  pull(logFC)

# Conduct KS enrichment testing
ks_res_NEM_3Y = impNEM_3Y %>% 
  group_by(Parent_haplotype) %>% 
...
print_res$NEM_3Y_No_filter = print(paste('Total of',
            ...
            'genes mapped to', 
            ks_res_NEM_3Y %>% pull(Parent_haplotype) %>% unique() %>% length(),
            'Subcellular loci '))
            
print_res$NEM_3Y_Filter = print(paste('and with fdr <= 0.05', 
            ks_res_NEM_3Y %>% filter(fdr < 0.05) %>% pull(logFC) %>% unique() %>% length(),
            'genes mapped to', 
            ks_res_NEM_3Y %>% filter(fdr < 0.05) %>% pull(Parent_haplotype) %>% unique() %>% length(),
            'Subcellular loci '))
```
...
# Make new table for combining DEG results with GO results and writing out
impNEM_3Y_Enrichment_out = plot_dat %>% 
  mutate(Trait = 'NEM_3Y')
...
NEM_3Y = ggplot(top, aes(x = Gene_type, y = reorder(Parent_haplotype, Avg_logFC), color = Avg_logFC))+
  geom_point(aes(size = log10(fdr)*-1, color = Avg_logFC))+
  geom_point(aes(size = log10(fdr)*-1), shape = 1, stroke = 0.25, color = 'black')+
...
length(unique(impLength_3Y$logFC))
```
...
```{r}
# Make vector of unique logFC values
ref_Length_3Y = impLength_3Y %>% 
  distinct(GeneID, logFC) %>% 
  pull(logFC)

# Conduct KS enrichment testing
ks_res_Length_3Y = impLength_3Y %>% 
  group_by(Parent_haplotype) %>% 
...
print_res$Length_3Y_No_filter = print(paste('Total of',
            ...
            'genes mapped to', 
            ks_res_Length_3Y %>% pull(Parent_haplotype) %>% unique() %>% length(),
            'Subcellular loci '))
            
print_res$Length_3Y_Filter = print(paste('and with fdr <= 0.05', 
            ks_res_Length_3Y %>% filter(fdr < 0.05) %>% pull(logFC) %>% unique() %>% length(),
            'genes mapped to', 
            ks_res_Length_3Y %>% filter(fdr < 0.05) %>% pull(Parent_haplotype) %>% unique() %>% length(),
            'Subcellular loci '))
```
...
# Make new table for combining DEG results with GO results and writing out
impLength_3Y_Enrichment_out = plot_dat %>% 
...
Length_3Y = ggplot(top, aes(x = Gene_type, y = reorder(Parent_haplotype, Avg_logFC), color = Avg_logFC))+
  geom_point(aes(size = log10(fdr)*-1, color = Avg_logFC))+
  geom_point(aes(size = log10(fdr)*-1), shape = 1, stroke = 0.25, color = 'black')+
...
```{r}
total_results = impCG_Enrichment_out %>% 
  rbind(impPHY_Enrichment_out) %>% 
  rbind(impNEM_3Y_Enrichment_out) %>% 
  rbind(impLength_3Y_Enrichment_out)

fwrite(total_results, 'Biological_Process_KS_results/Full_supplementary_results_KS.csv')
```
