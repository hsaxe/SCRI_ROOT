...
sjPlot::save_plot('DGEresults/CG_PHY_NEM_EBH.png', f, height = 14, width = 16)
```

# Read in Length results
```{r}
impLength = fread('DGEresults/Limma_results_table_Length.csv')
```

## Total Length DEGs by haplotype
```{r}
ASEsums = impLength %>% distinct(GeneID, Parent_haplotype) %>% count(Parent_haplotype)

p1 = ggplot(ASEsums, aes(Parent_haplotype, n, fill = Parent_haplotype))+
  geom_col(color = 'black')+
  theme(legend.position = 'none')+
  labs(y='DEGs')+
  ggtitle('Total Length DEGs')

```

# Length positive genes EBH
```{r}
impLength_pos = impLength %>%
  filter(logFC > 0) %>%
  distinct(GeneID, Parent_haplotype) %>% 
  count(Parent_haplotype)

p2 = ggplot(impLength_pos, aes(Parent_haplotype, n, fill = Parent_haplotype))+
  geom_col(color = 'black')+
  theme(legend.position = 'none')+
  labs(y='DEGs')+
  ggtitle('Length DEGs (+)')
...
# Length negative genes EBH
```{r}
impLength_neg = impLength %>%
  filter(logFC < 0) %>%
  distinct(GeneID, Parent_haplotype) %>% 
  count(Parent_haplotype)

p3 = ggplot(impLength_neg, aes(Parent_haplotype, n, fill = Parent_haplotype))+
  geom_col(color = 'black')+
  theme(legend.position = 'none')+
  labs(y='DEGs')+
  ggtitle('Length DEGs (-)')
...
# Put three Length plots together
```{r}
d = ggarrange(p1, p2, p3, nrow = 3)
