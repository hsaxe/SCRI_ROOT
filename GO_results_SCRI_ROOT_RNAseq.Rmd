...
#   coord_cartesian(clip = "off") +
#   # geom_text(data = labs, aes(label = `Cellular Component`), angle = 45)+
#   ggrepel::geom_label_repel(aes(label = `Cellular Component`, size = 2), box.padding = 0.4, label.padding = 0.1, max.overlaps = 30)+
...
dat = dat %>%
  ...
  mutate(`biological process` = gsub('\\(.*\\)', '', `biological process`),
    log2FC = log2(as.numeric(`fold Enrichment`)),
         `-log10 P-val` = log10(`raw P-value`)*-1,
    Direction = ifelse(
      log2FC >= 0, 'Pos', 'Neg'
    ))%>% 
  mutate(`biological process` = gsub('metabolic|process|processing|biosynthetic', '', `biological process`))
...
p = ggplot(dat, aes(log2FC, `-log10 P-val`))+
  geom_point(aes(size = log2FC^2, color = log2FC))+
  geom_point(aes(size = log2FC^2), shape = 1, color = 'black')+
  scale_y_log10()+
  coord_cartesian(clip = "off") +
...
save_plot('GOresults/GO_BP_UNITY_plot.png', p)
```
```
