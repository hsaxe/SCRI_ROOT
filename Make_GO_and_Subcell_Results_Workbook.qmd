---
title: "Make_GO_and_Subcell_KS_results_Workbook"
author: "Houston Saxe"
date: "8/15/2022"
...
# Read in results
```{r}
subcell_CG = fread('Subcell_KS_results/Subcell_KS_results_Table_CG.csv')

BP_CG = fread('Biological_Process_KS_results/BP_Results_Table_CG.csv')


subcell_PHY = fread('Subcell_KS_results/Subcell_KS_results_Table_PHY.csv')

BP_PHY = fread('Biological_Process_KS_results/BP_Results_Table_PHY.csv')


subcell_NEM = fread('Subcell_KS_results/Subcell_KS_results_Table_NEM.csv')

BP_NEM = fread('Biological_Process_KS_results/BP_Results_Table_NEM.csv')
```
...
NEM = fread('DGEresults/Limma_results_table_NEM_3Y.csv')%>% 
  mutate(Analysis = 'NEM')

Height = fread('DGEresults/Limma_results_table_Length_3Y.csv')%>% 
  mutate(Analysis = 'Height')
```
...
writeDataTable(wb, sheet = 1, x = CG, withFilter = T)

addWorksheet(wb, sheetName = 'Phytophtora DEGs', gridLines = F)

writeDataTable(wb, sheet = 2, x = PHY, withFilter = T)

addWorksheet(wb, sheetName = 'Nematode DEGs', gridLines = F)

writeDataTable(wb, sheet = 3, x = NEM, withFilter = T)

addWorksheet(wb, sheetName = 'Height DEGs', gridLines = F)

writeDataTable(wb, sheet = 4, x = Height, withFilter = T)

saveWorkbook(wb, 'Final_Figs_and_Draft_Manuscript_SCRI_ROOT/File S3.xlsx', 
             overwrite = T)
```
